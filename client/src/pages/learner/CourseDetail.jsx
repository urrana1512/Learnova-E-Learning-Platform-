...
const CourseDetail = () => {
  ...
  const [reviewXP, setReviewXP] = useState({
    show: false,
    earned: 0,
    total: 0,
  });
  ...
  const submitAnnouncement = async () => {
    ...
    try {
      await announcementAPI.create(id, announcementForm);
      toast.success("Broadcast transmitted successfully");
      setAnnouncementModal(false);
      setAnnouncementForm({ title: "", content: "", type: "INFO" });
      loadAnnouncements();
    } catch {
      toast.error("Shield failure: Broadcast transmission failed");
    } finally {
      setSubmittingAnnouncement(false);
    }
  };

  const { course, enrollment, lessonProgress, quizAttempts, leaderboard } =
    data || {};
  ...
  const allFinalDone =
    hasFinalAssignment && finalQuizzes.every((q) => completedQuizzes.has(q.id));
  ...
  const filteredQuizzes = course?.quizzes?.filter(
    (q) =>
      !lessonSearch ||
      q.title.toLowerCase().includes(lessonSearch.toLowerCase()),
  );
  ...
  const submitReview = async () => {
    if (!reviewForm.rating)
      return toast.error("Please select a mastery rating");
  ...
  const handleFollow = async () => {
    ...
    // Extract ID if instructorId is populated as an object
    const instrId = typeof course?.instructorId === "object"
    ...
    try {
      const res = await userAPI.toggleFollow(instrId);
  ...
  return (
    <LearnerLayout noFooter>
        ...
        {loading ? (
          ...
          >
            ...
            <h2 className="text-2xl font-black text-slate-900 font-sora">
              Course not localized
            </h2>
            <p className="text-slate-500 mt-2 max-w-sm">
              This module either does not exist or has been archived from the
              public database.
            </p>
                ...
                <div className="flex flex-col lg:flex-row items-end gap-12">
                  ...
                  <div className="flex-1 space-y-6">
                    ...
                    >
                      {course.tags?.map((t) => (
                        <span
                          key={t}
                          className="px-3 py-1 bg-white/5 border border-white/10 text-[9px] font-black text-slate-400 uppercase tracking-widest rounded-full"
                        >
                          {t}
                        </span>
                      ))}
                  ...
                  >
                    {hasAccess ? (
                      <>
                        <Link
                          to={`/learner/courses/${id}/learn/${course.lessons?.[0]?.id}`}
                        ...
                        {pct >= 100 && !hasFinalAssignment && (
                          <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl max-w-sm">
                            <p className="text-[10px] font-black text-amber-600 uppercase tracking-widest leading-relaxed">
                              ⚠️ Final Assessment Missing
                              <br />
                              <span className="font-medium lowercase first-letter:uppercase text-slate-500">
                                Each certified program requires a final mastery
                                exam. Please ask the instructor to add one.
                              </span>
                            </p>
                          </div>
                        )}
                      ...
                      <>
                        ...
                        <button
                          ...
                          className={`h-14 w-full lg:w-20 rounded-[1.5rem] border-2 flex items-center justify-center transition-all ${
                            isWishlisted
            ...
            <div className="max-w-7xl mx-auto px-6 py-12">
              <div className="flex flex-col lg:flex-row gap-16">
                ...
                <div className="flex-1 space-y-12">
                  <Tabs
                    tabs={[
                      { id: "overview", label: "Program Curriculum" },
                      ...
                      {
                        id: "reviews",
                        label: `Learner Feedback (${course.reviews?.length || 0})`,
                      },
                    ]}
                    activeTab={activeTab}
                    ...
                    {activeTab === "overview" && (
                      ...
                      >
                        <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
                          <h2 className="text-2xl font-black text-slate-900 font-sora tracking-tighter">
                            Content Delivery
                          </h2>
                          <div className="relative w-full sm:w-72">
                            ...
                            <input
                              ...
                              className="w-full h-12 pl-12 pr-4 bg-white border border-slate-200 rounded-2xl text-xs font-bold focus:ring-4 focus:ring-[#714B67]/5 transition-all outline-none"
                            />
                          </div>
                        </div>
                          ...
                          {filteredLessons?.map((lesson, i) => {
                            ...
                            return (
                              <motion.button
                                ...
                                onClick={() =>
                                  hasAccess &&
                                  navigate(`/learner/courses/${id}/learn/${lesson.id}`)
                                }
                                className={`w-full group flex items-center gap-6 p-6 rounded-[2rem] border transition-all duration-300 text-left relative overflow-hidden ${
                                    ...
                                    : "bg-white border-slate-200 hover:border-[#714B67]/40 hover:bg-slate-50"
                              ...
                              >
                                ...
                                <div className="flex-1 min-w-0">
                                  <p className="text-[10px] font-black text-[#714B67] uppercase tracking-widest mb-1.5 flex items-center gap-2">
                                    Module {i + 1} • {lesson.type}{" "}
                                    {lesson.duration &&
                                      `• ${formatDuration(lesson.duration)}`}
                                  </p>
                                  <h3
                                    className={`text-base font-black tracking-tight ${done ? "text-slate-300 line-through" : "text-slate-900"}`}
                                  >
                                    {lesson.title}
                                  </h3>
                                </div>
                                <div className="shrink-0 flex items-center gap-4">
                                  {done ? (
                                    ...
                                    <ChevronRight
                                      size={20}
                                      className="text-slate-300 group-hover:text-[#714B67] group-hover:translate-x-1 transition-all"
                                    />
                                  )}
                                </div>
                              </motion.button>
                          ...
                          {filteredQuizzes?.map((quiz, i) => {
                            ...
                            return (
                              <motion.button
                                ...
                                onClick={() =>
                                  hasAccess &&
                                  navigate(`/learner/courses/${id}/quiz/${quiz.id}`)
                                }
                                className={`w-full group flex items-center gap-6 p-6 rounded-[2rem] border transition-all duration-300 text-left relative overflow-hidden mt-6 ${
                                  !hasAccess
                                    ? "border-amber-500/10 bg-amber-500/5 cursor-not-allowed"
                              ...
                              >
                                ...
                                <div className="shrink-0 text-right">
                                  {done ? (
                                    <div>
                                      <p className="text-xl font-black text-amber-400 font-sora leading-none">
                                        {attempt.score}%
                                      </p>
                                      <p className="text-[10px] font-black text-amber-500/50 uppercase mt-1">
                                        Certified
                                      </p>
                                    </div>
                                  ) : (
                                    <Sparkles
                                      size={20}
                                      className="text-amber-400/50 group-hover:text-amber-500 animate-pulse"
                                    />
                                  )}
                                </div>
                              </motion.button>
                      ...
                      >
                        <div className="flex items-center justify-between mb-8">
                          ...
                          <div className="flex items-center gap-4">
                            {loadingAnnouncements && <Spinner size="sm" />}
                            {isInstructorOrAdmin && (
                              <Button
                        ...
                        {announcements.length > 0
                          ? announcements.map((ann) => (
                              <div
                                key={ann.id}
                                className="p-8 bg-white border border-slate-200 rounded-[2rem] shadow-sm relative overflow-hidden group"
                              >
                                ...
                                <h3 className="text-xl font-black text-slate-900 font-sora mb-3">
                                  {ann.title}
                                </h3>
                        ...
                        {allResources.length > 0 ? (
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {allResources.map((res) => (
                              <div
                                key={res.id}
                                className="p-6 bg-white border border-slate-200 rounded-[2rem] flex items-center gap-4 hover:border-[#714B67]/20 transition-all group"
                              >
                                <div className="w-12 h-12 rounded-2xl bg-slate-50 flex items-center justify-center text-[#714B67] shrink-0 border border-slate-100 group-hover:scale-110 transition-transform">
                                  <Download size={20} />
                                </div>
                                <div className="flex-1 min-w-0">
                                  ...
                                  <h4 className="text-sm font-black text-slate-900 truncate font-sora">
                                    {res.name}
                                  </h4>
                                </div>
                                <a
                                  ...
                                  className="px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-600 hover:bg-[#714B67] hover:text-white transition-all shadow-sm"
                                >
                                  Download
                                </a>
                              </div>
                          ...
                          <div className="text-center py-24 bg-white border-2 border-dashed border-slate-100 rounded-[2.5rem]">
                            ...
                            />
                            <p className="text-slate-400 font-black uppercase text-xs tracking-widest">
                              No additional resources identified for this
                              module.
                            </p>
                          </div>
                        )}
                      ...
                      >
                        ...
                        <div className="bg-white border border-slate-200 rounded-[2.5rem] p-10 flex flex-col md:flex-row items-center gap-16 relative overflow-hidden shadow-sm">
                          ...
                          <div className="text-center relative z-10">
                            <p className="text-7xl font-black text-slate-900 font-sora tracking-tighter mb-2">
                              {avgRating || "0.0"}
                            </p>
                            ...
                            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">
                              {course.reviews?.length || 0} Peer Reviews
                            </p>
                          </div>
                          ...
                          <div className="flex-1 space-y-4 relative z-10 w-full">
                            {[5, 4, 3, 2, 1].map((n) => {
                              const count =
                                course.reviews?.filter((r) => r.rating === n)
                                  .length || 0;
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <div className="flex-1 h-2 bg-slate-50 rounded-full overflow-hidden p-0.5 border border-slate-100">
                                    <div
                                      className="h-full bg-[#714B67]/80 rounded-full"
                                      style={{
                          ...
                          {enrollment && (
                            <Button
                              ...
                              variant="primary"
                              className="h-16 px-10 rounded-2xl relative z-10"
                            >
                              Submit Feedback
                            </Button>
                          )}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          {course.reviews?.map((r, i) => (
                            <div
                              key={r.id}
                              className="p-8 bg-white border border-slate-200 rounded-[2rem] hover:border-[#714B67]/20 transition-all shadow-sm"
                            >
                          ...
                          {(!course.reviews || course.reviews.length === 0) && (
                            <div className="col-span-2 text-center py-20 bg-white rounded-[2.5rem] border-dashed border-2 border-slate-100">
                              ...
                              <p className="text-slate-500 font-black uppercase text-xs tracking-widest">
                                No feedback synchronized for this module yet.
                              </p>
                            </div>
                          )}
                        </div>
                ...
                <div className="w-full lg:w-[350px] shrink-0 space-y-8">
                      ...
                      <div className="space-y-6">
                        {[
                          ...
                          <div key={i} className="flex items-center gap-4">
                            <div
                              className={`w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center ${stat.color} border border-slate-100`}
                            >
                              <stat.icon size={18} />
                            </div>
                            <div>
                              <p className="text-xs font-black text-slate-600 uppercase tracking-widest leading-none mb-1">
                                {stat.label}
                              </p>
                    ...
                    {enrollment && (
                      <div className="pt-8 border-t border-slate-100 space-y-4">
                        <div className="flex justify-between items-end mb-2">
                          ...
                          <p className="text-lg font-black text-slate-900 font-sora">
                            {pct}%
                          </p>
                        </div>
                        <div className="h-3 bg-slate-50 rounded-full overflow-hidden p-0.5 border border-slate-100">
                          <motion.div
                            ...
                            className="h-full bg-gradient-to-r from-[#714B67] to-[#017E84] rounded-full shadow-[0_0_15px_rgba(113,75,103,0.3)]"
                          />
                        </div>
                      </div>
                    )}
                    ...
                    {!enrollment && (
                      <div className="bg-[#714B67]/5 border border-[#714B67]/10 rounded-2xl p-6">
                        <p className="text-xs font-bold text-[#714B67] leading-relaxed">
                          Secure your access profile today to begin tracking
                          your evolution and earn verified XP milestones.
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  {leaderboard && leaderboard.length > 0 && (
                    <div className="p-8 bg-white border border-slate-200 rounded-[2.5rem] relative overflow-hidden shadow-sm">
                      <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest mb-6 relative z-10 flex items-center gap-2">
                        <Trophy size={16} className="text-amber-500" /> Top
                        Scholars
                      </h3>
                        ...
                        {leaderboard.map((attempt, idx) => (
                          ...
                          >
                            ...
                            <div className="text-right shrink-0">
                              <p className="text-xs font-black text-[#714B67] font-sora">
                  ...
                  <div className="p-8 bg-white border border-slate-200 rounded-[2.5rem] relative overflow-hidden group shadow-sm">
                    ...
                    <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest mb-6 relative z-10">
                      Academy Lead
                    </h3>
                    <div className="flex items-center gap-4 mb-6 relative z-10">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] font-black text-[#017E84] uppercase tracking-widest mt-1 italic">
                          Verified Mentor
                        </p>
                      </div>
                    </div>
                    <p className="text-xs font-medium text-slate-500 leading-relaxed mb-6 relative z-10">
                      Driving innovation and mastery across the global learning
        ...
        <div className="space-y-10">
          ...
          <div className="flex flex-col gap-3">
            ...
            <button
              ...
              className="h-12 text-[10px] font-black text-slate-600 uppercase tracking-widest hover:text-white transition-colors"
            >
              Discard Draft
            </button>
          </div>
      ...
      <Modal
        ...
        className="max-w-xl bg-white border border-slate-200 rounded-[2.5rem] p-10"
      >
        <div className="space-y-8">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <select
                ...
                onChange={(e) =>
                  setAnnouncementForm((f) => ({ ...f, type: e.target.value }))
                }
                className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl h-14 px-4 text-xs font-black uppercase focus:outline-none focus:border-[#714B67]/20 transition-all"
              >
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                placeholder="Mastery Update..."
                className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl h-14 px-4 text-sm font-medium focus:outline-none focus:border-[#714B67]/20 transition-all"
              />
            </div>
          </div>
          <div className="space-y-2">
            ...
            <textarea
              ...
              className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl p-6 text-sm font-medium focus:outline-none focus:border-[#714B67]/20 transition-all resize-none shadow-inner"
            />
          </div>
          <div className="flex flex-col gap-3 pt-4">
            ...
            <button
              ...
              className="h-12 text-[10px] font-black text-slate-600 uppercase tracking-widest"
            >
              Abort Mission
            </button>
          </div>
