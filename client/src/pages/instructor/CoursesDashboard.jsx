...
const CourseCard = ({
  ...
  const totalDuration =
    course.lessons?.reduce((s, l) => s + (l.duration || 0), 0) || 0;
  ...
  const handleCreate = async () => {
    ...
    try {
      const { data } = await courseAPI.create({
        title: newTitle.trim(),
        rewardXP: parseInt(newRewardXP) || 500,
      });
      setNewModal(false);
      setNewTitle("");
      setNewRewardXP(500);
    ...
    <AdminLayout>
              ...
              onClick={() =>
                exportCSV(
                  filtered.map((c) => ({
                    ...
                    "Duration (min)":
                      c.lessons?.reduce((s, l) => s + (l.duration || 0), 0) ||
                      0,
                  })),
                  "learnova_courses",
                )
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div>
              <h3 className="text-xs font-bold text-slate-400 mb-4 uppercase tracking-widest pl-1">
                Draft{" "}
                <span className="bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full ml-1">
                  {drafts.length}
                </span>
              </h3>
              <div className="space-y-3">
                {drafts.map((c) => (
                  <CourseCard
                    ...
                    onAttendees={(id) =>
                      navigate(`/admin/courses/${id}/attendees`)
                    }
                ...
                {filtered.map((c) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-center">
                      <Badge variant={c.isPublished ? "green" : "slate"}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <Button
                          ...
                          icon={<Users size={13} />}
                          onClick={() =>
                            navigate(`/admin/courses/${c.id}/attendees`)
                          }
                          className="text-[#714B67]"
                          title="Attendees"
                        />
      ...
      >
        <p className="text-slate-600 text-sm mb-4">
          ...
          This will also delete all lessons and quizzes.
        </p>
