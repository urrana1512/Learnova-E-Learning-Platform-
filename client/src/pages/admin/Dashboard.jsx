    ...
    <AdminLayout>
      <div className="p-6 max-w-7xl mx-auto font-inter">
        <header className="mb-8 flex items-end justify-between">
          <div>
            <p className="text-[10px] font-black uppercase tracking-widest text-[#017E84] mb-1">
              Status Report
            </p>
            <h1 className="text-3xl font-bold font-sora text-[#714B67]">
              Platform Overview
            </h1>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm relative overflow-hidden group">
            ...
            <p className="text-4xl font-black text-slate-900 font-sora tracking-tight">
              {stats?.platform?.totalEnrollments || 0}
            </p>
          </div>
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm relative overflow-hidden group border-b-4 border-b-emerald-500/10">
            ...
            <div className="flex items-baseline gap-2">
              <p className="text-4xl font-black text-slate-900 font-sora tracking-tighter">
                ₹{(stats?.platform?.totalRevenue || 0).toLocaleString()}
              </p>
              <span className="text-[10px] font-black text-emerald-500 flex items-center bg-emerald-50 px-2 py-0.5 rounded-full">
                +12.4%
              </span>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm relative overflow-hidden group">
            ...
            <p className="text-4xl font-black text-slate-900 font-sora tracking-tight">
              {user?.role === "ADMIN"
                ? stats?.platform?.totalNetworkUsers || 0
                : stats?.platform?.totalEnrollments || 0}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            <h3 className="text-sm font-bold text-slate-800 mb-6 flex items-center gap-2">
              <Activity size={18} className="text-[#017E84]" /> Engagement
              Distribution
            </h3>
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            <h3 className="text-[10px] font-black uppercase tracking-widest text-[#714B67] mb-6">
              ...
              <div className="flex items-center gap-4 p-4 rounded-2xl bg-slate-50 border border-slate-100 hover:bg-slate-100 transition-colors">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">
                    All platform microservices are responding accurately.
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-4 p-4 rounded-2xl bg-slate-50 border border-slate-100 hover:bg-slate-100 transition-colors">
