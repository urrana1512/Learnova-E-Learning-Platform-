...
const UsersDashboard = () => {
  ...
  const handleToggleStatus = async (user) => {
    ...
    setToggling(user.id);
    try {
      const { data } = await adminUsersAPI.toggleStatus(user.id, {
      ...
      toast.success(
        `User successfully ${data.isActive ? "restored" : "banned"}.`,
      );
    } catch {
  ...
  const handleDelete = async (user) => {
    ...
    if (
      !window.confirm(
        `Are you sure you want to explicitly ban and permanently remove ${user.name}? This cannot be undone.`,
      )
    )
      return;
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-100 text-slate-400 text-[10px] uppercase tracking-widest bg-slate-50 relative">
                    ...
                    <th className="text-center px-6 py-4 font-black hidden lg:table-cell">
                      Engagement
                    </th>
                    <th className="text-right px-6 py-4 font-black">
                      Total XP
                    </th>
                    <th className="text-center px-6 py-4 font-black">
                      Actions
                    </th>
                  </tr>
                </thead>
                          ...
                          {u.id !== currentUser.id ? (
                            <>
                              <Button
                                ...
                                className={
                                  u.isActive
                                    ? "text-orange-500 hover:bg-orange-50 hover:text-orange-600 rounded-xl"
                                    : "bg-emerald-500 hover:bg-emerald-600 border-none shadow-md shadow-emerald-500/20 rounded-xl"
                                }
                              >
